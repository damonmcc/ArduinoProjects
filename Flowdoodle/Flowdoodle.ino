// Sketch for Flowdoodle 3D printer LEDS.  Designed for
...
boolean  prevStateSwitch, prevStateRight, switchPressed;
uint32_t lastChangeTimeSwitch = 0, lastChangeTimeRight = 0, intensity = 1;
...
//  FastLED.setBrightness( BRIGHTNESS );
  ...
  Serial.begin(9600);
  digitalWrite(analogInPin, INPUT_PULLUP);
  prevStateSwitch  = false; // Initial button states
  switchPressed = false;
...
//  Serial.println(sensorValue);
  ...
  uint32_t t = millis();
  if((t - lastChangeTimeSwitch) >= DEBOUNCE_MS) {
    boolean b = switchPressed;
    if(b != prevStateSwitch) {             // Left button state changed?
      prevStateSwitch      = b;
      lastChangeTimeSwitch = t;
      if(b) {                            // Left button pressed?
        if(intensity) intensity--;                 // Go to prior mode
        else     intensity = 2;     // or "wrap around" to last mode
...
void mode_flowdoodle() {
  ...
  uint32_t c = pgm_read_byte(&gammaTable[y]) * x;
  for(uint8_t i=0; i<NUM_LEDS; i++) {
//    leds[i] = CRGB(c);
    strip.setPixelColor(i, c);
  }

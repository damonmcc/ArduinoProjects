// SA node in right atrium at P wave
// AV node delays for PR segment
// Purkinje fibers at peak QRS complex
// Arrhythmia: resting heat rate < 60 or > 100 BPM
...
void checkPulse(){
  if (analogRead(ECG_PIN)>Threshold){          //If current channel reading exceeds threshold
    Hit = AnalogMaxima (ECG_PIN,Threshold,2);  //find local maxima and return value.
    if (Hit == 1025 || Hit == 0){        //if still ascending or below threshold,
      ...
      Serial.println(Hit);               //Show maxima
      Pulsed = false;
      if(Hit>Max){
        Max = Hit;
        Pulsed = true;
...
int AnalogMaxima (int AnalogCh, int Threshold, int Delay){    
  ...
  check1 = analogRead(AnalogCh);               //Assign first reading 
  delay(Delay);                                //wait
...
// Perceptually half-brightness gamma-corrected sine wave simulating the 
// electrical activity of a beating heart. Timing is based on the calculated
// BPM of the PQRST complex detected with an analog input.
void mode_heart() {
  checkPulse();
  if(Pulsed){
    uint32_t t = millis() / 4;
